...
def find_best_match(recognized_text, string_list):
    best_match = None
    highest_ratio = 0

    for string in string_list:
        ratio = fuzz.ratio(recognized_text.lower(), string.lower())
...
attachment_dir = 'C:/Users/gupta/Documents/'
...
#         message['To'] = 'dbz26102001@gmail.com',
#         message['From'] = 'guptarohandec@gmail.com'
...
# <------------------------------------Changed------------------------------------>
def speechtotext(duration,string_list):
    ...
    try:
        ...
        if string_list is not None:
            best_match = find_best_match(response, string_list)
            response=best_match
...
def speechtotext_compose(duration,string_list):
    global i, addr, passwrd
    r = sr.Recognizer()
    with sr.Microphone() as source:
        r.adjust_for_ambient_noise(source, duration=1)
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        while (flag):
            ...
            i = i + str(1)
            # <------------------------------------Changed------------------------------------>
            addr = speechtotext(10,email_list)
            # <------------------------------------Changed------------------------------------>
            ...
            if addr != 'N':
                ...
                i = i + str(1)
                # <------------------------------------Changed------------------------------------>
                say = speechtotext(3,None)
                # <------------------------------------Changed------------------------------------>
                if say == 'yes' or say == 'Yes' or 'Yes.':
                    flag = False
            else:
        ...
        while (flag):
            ...
            i = i + str(1)
            # <------------------------------------Changed------------------------------------>
            passwrd = speechtotext(10,password_list)
            # <------------------------------------Changed------------------------------------>
...
def compose_view(request):
    global i, addr, passwrd, s, item, subject, body, to
    SCOPES = ['https://www.googleapis.com/auth/gmail.readonly',
          "https://mail.google.com/",
          "https://www.googleapis.com/auth/gmail.modify",
          "https://www.googleapis.com/auth/gmail.compose",
    ...
    if request.method == 'POST':
        ...
        # i = i + str(1)
        # to = ""
        # to = speechtotext_compose(15, email_list)
        flag = True
        while (flag and to != 'N'):
            texttospeech("enter receiver's email address:", file + i)
            i = i + str(1)
        ...
        while (flag):
            ...
            i = i + str(1)
            subject = speechtotext_compose(10, None)
            if subject == 'N':
                texttospeech("could not understand what you meant", file + i)
                i = i + str(1)
            else:
        ...
        while flag:
            ...
            i = i + str(1)
            body = speechtotext_compose(20, None)
            if body == 'N':
                texttospeech("could not understand what you meant", file + i)
                i = i + str(1)
            else:
        ...
        if x == 'yes':
            ...
            if say == 'yes':
                ...
                i = i + str(1)
                audio_msg = speechtotext_compose(10, None)
                flagconf = True
                while flagconf:
                    try:
        ...
        i = i + str(1)
        return JsonResponse({'result': 'success'})
            # return JsonResponse({'result': 'failure', 'error_message': error_message})
        ...
        #     error_message = str(e)
        #     print(f"Error occurred while sending email: {error_message}")
        #     texttospeech("Sorry, your email failed to send. Please try again. You will now be redirected to the compose page again.", file + i)
        #     i = i + str(1)
        #     return JsonResponse({'result': 'failure', 'error_message': error_message})
        # s.quit()
        # return JsonResponse({'result' : 'success'})
